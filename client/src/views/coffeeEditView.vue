...
onMounted(async () => {
  ...
  const coffee = await store.getOne(id);
  form.value = {
...
async function handleSubmit() {
  ...
  try {
    ...
    router.push({ name: "coffee", params: { id } });
  } catch {
    error.value = "Failed to update coffee bag";
  } finally {
    loading.value = false;
  }
}
    ...
    <div class="container">
      <div class="level">
        <div class="level-left">
          <h1 class="title">Edit Coffee Bag</h1>
        </div>
        <div class="level-right">
          <RouterLink :to="`/coffees/${route.params.id}`" class="button">Back</RouterLink>
        </div>
      </div>
      <div class="box">
